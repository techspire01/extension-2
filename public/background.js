...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  const stored = await chrome.storage.local.get([BOARD_KEY, NOTIFICATION_LOG_KEY]);
  ...
  for (const task of board.tasks || []) {
    if (!task.eta || task.status === 'done') continue;
    ...
    if (board.settings.etaReminders && remaining > 0 && remaining <= reminderWindow && !log[dueSoonKey]) {
  ...
  if (board.settings.sprintReminders) {
    for (const sprint of board.sprints || []) {
      ...
      if (remaining > 0 && remaining <= 24 * 3600000 && !log[key]) {
        const incomplete = (board.tasks || []).filter((task) => task.sprintId === sprint.id && task.status !== 'done').length;
