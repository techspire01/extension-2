...
const STATUSES: {
  id: Exclude<ScrumStatus, "backlog">;
  label: string;
  accent: string;
}[] = [
  { id: "todo", label: "To do", accent: "#579dff" },
  { id: "in-progress", label: "In progress", accent: "#f5cd47" },
  { id: "done", label: "Done", accent: "#4bce97" },
];
...
export const ScrumWorkspace: React.FC<ScrumWorkspaceProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-[#f7f8f9] text-[#172b4d] flex overflow-hidden">
      <aside className="hidden md:flex w-60 bg-[#172b4d] text-white flex-col shrink-0">
        <div className="h-16 flex items-center gap-3 px-5 border-b border-white/10">
          ...
          <div>
            ...
            <div className="text-[10px] text-[#b6c2cf]">Personal workspace</div>
          </div>
        </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 bg-white border-b border-[#dfe1e6] flex items-center gap-3 px-3 sm:px-5 shrink-0">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            <button
              onClick={() =>
                openCreate(view === "backlog" ? "backlog" : "todo")
              }
                ...
                <div className="scrum-popover right-0 w-80 p-4 space-y-4">
                  ...
                  <div className="space-y-2 pl-2 border-l-2 border-[#dfe1e6]">
                    <label className="flex justify-between text-xs font-semibold">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          board.updateSettings({ etaReminders: e.target.checked })
                        }
                      />
                    </label>
                    <label className="flex justify-between text-xs font-semibold">
                      ...
                      <input
                        ...
                        disabled={!data.settings.notificationsEnabled}
                        onChange={(e) =>
                          board.updateSettings({ overdueReminders: e.target.checked })
                        }
                      />
                    </label>
                    <label className="flex justify-between text-xs font-semibold">
                      ...
                      <input
                        ...
                        disabled={!data.settings.notificationsEnabled}
                        onChange={(e) =>
                          board.updateSettings({ sprintReminders: e.target.checked })
                        }
                      />
                    </label>
                  </div>
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        board.updateSettings({ reminderMinutes: Number(e.target.value) })
                      }
                    >
                  ...
                  <label className="block">
                    <span className="scrum-label">When completing a sprint</span>
                    <select
                      className="scrum-input"
                      value={data.settings.incompleteSprintAction}
                      onChange={(e) =>
                        board.updateSettings({
                          incompleteSprintAction: e.target.value as typeof data.settings.incompleteSprintAction,
                        })
                      }
                    >
                      <option value="backlog">Return incomplete tasks to backlog</option>
                      <option value="next-sprint">Move incomplete tasks to next sprint</option>
                    </select>
                  </label>
        ...
        {showFilters && (
          <section className="px-5 py-3 bg-[#f7f8f9] border-b border-[#dfe1e6] flex flex-wrap gap-4 items-end">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {[
                  { id: "backlog" as ScrumStatus, label: "Backlog" },
                  { id: "todo" as ScrumStatus, label: "To do" },
                  { id: "in-progress" as ScrumStatus, label: "In progress" },
                  { id: "done" as ScrumStatus, label: "Done" },
                ].map((status) => (
                  <button
                    key={status.id}
                    onClick={() =>
                  ...
                  <button
                    onClick={() => {
                      ...
                      const shouldMoveToNext =
                        data.settings.incompleteSprintAction === "next-sprint" &&
                        nextSprint;
                      if (
                        confirm(
                        ...
                        {activeSprint && (
                          <button
                            onClick={(event) => {
                              ...
                              board.updateTask(
                                task.id,
                                { sprintId: activeSprint.id, status: "todo" },
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSprintDraft({ ...sprintDraft, endDate: e.target.value })
                  }
                />
              </label>
            </div>
